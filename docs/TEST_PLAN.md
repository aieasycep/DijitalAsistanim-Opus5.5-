...
| UT-NTF-13 | Property (fast-check) over all categories and entities | `title_only` and `generic` never contain a contact name, subject, amount or email (= TST-EF-12); data keys exactly `{type, entity_id, deeplink}`; payload < 1 KB |
...
| IT-NTF-05 | Payload shape | `channelId` per category (R-12 ids, UT-NTF-14); `interruptionLevel` per §2.12; data keys exactly `{type, entity_id, deeplink}`; body rendered for the user's `notification_detail` |
...
- **Harness** (`scripts/e2e/harness-server.ts`, Node, `127.0.0.1:8790`; CI only):
  ...
  - Endpoints:
    ...
    - `POST /revenuecat/activate {userKey, product}` → makes the mock REST v2 return active for that customer (CI only). The harness looks up the user's auth id (the RevenueCat app user id) and posts `{app_user_id, product}` to the mock provider server's `/revenuecat/__activate` (`REVENUECAT_MOCK_URL`). `scripts/e2e/start-stack.sh` starts that server on the Docker bridge gateway, which the edge runtime container reaches as `host.docker.internal` (`REVENUECAT_API_BASE_URL`, with a run-scoped `REVENUECAT_API_V2_SECRET_KEY`).
...
| WEB-E2E-08 | `oauth-done.spec.ts` | `/oauth/done?provider=google&result=pending_confirmation&completion_code=…` renders "Uygulamaya dön" with a deep link that forwards the query to `dijitalasistan://integrations/callback` (the app then calls API-INT-07, R-07); the page never renders the code or any token as text; `Referrer-Policy: no-referrer`, `Cache-Control: no-store`, `noindex` |
...
| `mobile-checks` | all | install | ubuntu | `pnpm --filter mobile exec expo install --check`; `expo-doctor`; `expo export --platform ios --platform android`; `expo prebuild --clean --no-install` (Android and iOS) into temp dirs; Android manifest assertions (= TST-CI-07); EAS config lint (UT-ENV-08) | Metro cache | exported bundles (for secret-scan) |
