 ...
 *
 * The keys of the first seed (T-5.16) render `20260924002410_prompt_versions_seed.sql`; every later
 * key (T-5.09…T-5.15: meeting, assistant, reply and capture prompts) renders the part-2 seed
 * `20260924002610_prompt_versions_seed_part2.sql`, so an applied migration never changes.
 *
...
export const MIGRATION_PART2 = join(
  ROOT,
  'supabase',
  'migrations',
  '20260924002610_prompt_versions_seed_part2.sql',
);
/** Prompt keys seeded by the first migration; every other key goes to the part-2 migration. */
export const PART1_KEYS: readonly string[] = [
  'briefing_evening',
  'briefing_midday',
  'briefing_morning',
  'commitment',
...
export function renderMigration(
  ...
  title = 'Prompt versions v1 (IMPLEMENTATION_PLAN T-5.16; AI_PIPELINE_PLAN §5.2, §5.4).',
): string {
...
/** The migration files and their rendered SQL (part 1: `PART1_KEYS`, part 2: every other key). */
export function renderParts(prompts: readonly PromptSource[]): { file: string; sql: string }[] {
  const first = prompts.filter((p) => PART1_KEYS.includes(p.prompt_key));
  const second = prompts.filter((p) => !PART1_KEYS.includes(p.prompt_key));
  return [
    { file: MIGRATION, sql: renderMigration(first) },
    {
      file: MIGRATION_PART2,
      sql: renderMigration(
        second,
...
function main(): void {
  ...
  const parts = renderParts(loadPrompts());
  if (check) {
    for (const part of parts) {
      let current = '';
      try {
        current = readFileSync(part.file, 'utf8');
      } catch {
        current = '';
      }
      if (current !== part.sql) {
  ...
  for (const part of parts) {
    writeFileSync(part.file, part.sql);
    process.stdout.write(`gen-prompt-migration: wrote ${part.file}\n`);
  }
}
