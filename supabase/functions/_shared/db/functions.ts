 ...
 *
 * Functions in `private` are documented as `service_role` only. PostgREST exposes the schemas listed
 * in `supabase/config.toml` (`public`, `admin_api`), so calls to `private.*` succeed once that schema
 * is reachable for the service role or a `public` service-role wrapper with the same name and
 * arguments exists; switching is a one-line change of `schema` below.
 */
...
export const DB_FN = {
  ...
  userAppleSub: fn('private', 'user_apple_sub', '(p_user uuid) returns text'),
  consumeProviderQuota: fn(
    'private',
    'consume_provider_quota',
    '(p_bucket text, p_account uuid, p_units int, p_limit int, p_window_seconds int) returns int (wait_ms)',
  ),
  auditVerifyChain: fn(
    'private',
    'audit_verify_chain',
