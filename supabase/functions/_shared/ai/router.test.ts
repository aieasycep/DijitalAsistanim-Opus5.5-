...
Deno.test(
  'supabase sources: config rows cached for 60 s, breaker via public.ai_breaker_state, profile from plan_limits',
  async () => {
    let t = 0;
    ...
    assertEquals(breakerCall?.headers.get('Content-Profile'), 'public');
