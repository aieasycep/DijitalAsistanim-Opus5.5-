...
Deno.test('rows are persisted for every probe with checked_by and the correlation id', async () => {
  ...
  assert(rows.some((r) => r.component === 'email_delivery'));
  assert(rows.some((r) => r.component === 'audit_chain'));
  assert(results.some((r) => r.probe === 'email_delivery'));
  assert(results.some((r) => r.probe === 'audit_chain'));
  assertEquals(
    sink.records().filter((r) => r.msg === 'health_probe_unpersisted'),
    [],
  );
});
...
Deno.test('health rejects invalid run bodies and browser origins', async () => {
  ...
  const bad = await app.request('/health/run', {
    method: 'POST',
    headers: { apikey: raw.CRON_SECRET ?? '', 'Content-Type': 'application/json' },
    body: JSON.stringify({ probes: ['worker'] }),
  });
