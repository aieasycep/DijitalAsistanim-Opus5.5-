 ...
 *
 * The `Database` type is supplied by the caller (the generated `database.types.ts`). Until it
 * exists, callers pass `EmptyDatabase`, which types every PostgREST call as an error: only Auth and
 * Functions are usable, and no untyped table access can slip in.
 */
import {
  createClient,
  type SupabaseClient,
